...
TEST_RESULTS_DIR = os.path.join(
    os.path.dirname(__file__), "test_results", "preprocess"
)
...
class TestPreprocessImage:
    ...
    def test_preprocess_image_output(self, image_name):
        ...
        assert isinstance(original, np.ndarray), "L'image originale doit être un tableau numpy"
        assert original.ndim == 3 and original.shape[2] == 3, "L'image originale doit être en couleur (3 canaux)"

        # Vérifie que l'image prétraitée est bien générée
        assert edged is not None, "L'image prétraitée ne doit pas être None"
        assert isinstance(edged, np.ndarray), "L'image prétraitée doit être un tableau numpy"
        assert edged.ndim == 2, "L'image prétraitée doit être une image binaire ou en niveaux de gris (2D)"

        # Sauvegarde les résultats pour inspection visuelle
        image_base = os.path.splitext(image_name)[0]
        cv2.imwrite(os.path.join(TEST_RESULTS_DIR, f"{image_base}_original.png"), original)
