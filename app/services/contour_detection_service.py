...
def _detect_cards_with_method(image_path, method, min_area, aspect_ratio_range, debug, output_dir, common_output_dir, orig_image, image_area):
    """
    ...
    for i, cnt in enumerate(contours):
        ...
        if len(approx) == 4 and cv2.isContourConvex(approx):
            ...
            # print(f"  ACCEPTÉ (Méthode {method}) : Contour n°{i+1} détecté comme carte !")
            ...
            if debug and output_dir:
                ...
                cv2.imwrite(
                    os.path.join(output_dir, f"{image_name}_card_{method}_{i}.png"), warped
                )
        ...
        # else:
            # print(f"  REJETÉ (Méthode {method}) : Pas 4 coins (a {len(approx)} coins) ou non convexe.")
...
def detect_cards(
    ...
    image, _ = preprocess_image(image_path, method="canny") # On charge l'image originale une seule fois
    orig = image.copy()
    h, w = image.shape[:2]
    image_area = h * w
    ...
    if method != "canny": methods_to_try.append("canny") # S'assurer que canny est essayé si ce n'est pas la première
    if "light" not in methods_to_try: methods_to_try.append("light")
    if "spec" not in methods_to_try: methods_to_try.append("spec")
    ...
    for current_method in methods_to_try:
        if not final_card_contours: # Seulement si aucune carte n'a encore été trouvée
            ...
            if temp_card_contours:
                ...
                print(f"Méthode '{current_method}' a détecté {len(final_card_contours)} carte(s). Arrêt de la recherche.")
