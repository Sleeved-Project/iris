...
def preprocess_image(image_path, method="canny"):
    ...
    processing_configs = {
        ...
        "spec": {
            "gaussian_blur_kernel": (9, 9), # Peut être ajusté si "spec" a besoin de plus/moins de flou
            "canny_threshold1": 30,
    ...
    if config is None:
        raise ValueError(f"Méthode de prétraitement '{method}' non supportée. Méthodes disponibles : {list(processing_configs.keys())}")
